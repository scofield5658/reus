...
import { createFromTemplate } from '../dist/cli/commands/create-core.js';
import { spawnAndWait } from '../dist/cli/commands/process.js';
...
test('launch help documents prod as the default mode', async () => {
  const { stdout } = await execFileAsync(
    process.execPath,
    ['dist/cli/command.js', 'launch', '--help'],
    { cwd: repositoryRoot },
  );
...
test('build produces dist and exits successfully', async (t) => {
  ...
  await execFileAsync(
    process.execPath,
    ['dist/cli/command.js', 'build', fixture.projectDir],
    { cwd: repositoryRoot },
  );
...
test('launch keeps prod as the effective default mode', async (t) => {
  ...
  const launched = spawn(
    process.execPath,
    ['dist/cli/command.js', 'launch', fixture.projectDir],
    {
      cwd: repositoryRoot,
